...
import { nowISOString, generateUID, vaultSave } from "../../../core/utils";
...
export class RecallCardStore {
  ...
  // 卡片更新回调（用于刷新向量）
  private onCardUpdatedCallback?: (card: RecallCard) => void;
  ...
  setOnCardUpdatedCallback(callback: (card: RecallCard) => void) {
    this.onCardUpdatedCallback = callback;
  }
  ...
  async saveCard(card: RecallCard): Promise<void> {
    ...
    // 触发卡片更新回调（新卡片或内容变更时刷新向量）
    if (this.onCardUpdatedCallback) {
      this.onCardUpdatedCallback(card);
    }
  }
  ...
  async getAllCards(scenario?: RecallScenario): Promise<RecallCard[]> {
    ...
    const root = this.app.vault.getAbstractFileByPath(base);
    if (!root) return result;

    const walk = async (folder: any) => {
      for (const child of folder.children || []) {
        if (child.children) {
          await walk(child);
