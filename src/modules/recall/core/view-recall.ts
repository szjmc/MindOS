...
import { CardRelationView } from "../card-relations/card-relation-view";
...
export class RecallView {
  ...
  // ✅ 修复：防止 re-render 打断输入
  private renderDebounceTimer: number | null = null;
  private lastRenderState: string = ""; // 缓存上次渲染的状态摘要
  ...
  render(parent: HTMLElement) {
    ...
    // ✅ 修复：生成当前状态摘要，用于判断是否需要重新渲染
    const currentRenderState = this.buildRenderStateHash(state);
    const activeEl = document.activeElement as HTMLElement | null;
    const isInInput = activeEl?.tagName === "TEXTAREA" || activeEl?.tagName === "INPUT";
    
    // 如果在输入框中且状态没有本质变化，跳过渲染避免打断输入
    if (isInInput && this.answerInputEl && currentRenderState === this.lastRenderState) {
      return;
    }
    this.lastRenderState = currentRenderState;

    let savedAnswer: { selectionStart: number; selectionEnd: number; value: string } | null = null;
    if (isInInput && this.answerInputEl) {
      savedAnswer = {
  ...
  /**
   * ✅ 生成渲染状态摘要，用于判断是否需要重新渲染
   * 只包含影响 UI 的关键状态，排除日志等频繁变化的内容
   */
  private buildRenderStateHash(state: any): string {
    const keyParts = [
      state.viewMode,
      state.selectedScenario,
      state.currentSession?.id,
      state.currentSession?.cards?.length,
  ...
  private attachKeyboardListener() {
    ...
    this.keyboardHandler = (e: KeyboardEvent) => {
      ...
      if (!state.isFlipped) {
        // vocab 场景自定义朗读快捷键
        const isVocab = state.currentCard.scenario === "vocab";
        const isAutoSpeak = isVocab && this.plugin.settings.recallVocabAutoSpeak;
        ...
        // ✅ autoSpeak 开启后：Space = 重读，Enter = 翻转
        if (isAutoSpeak && e.code === "Space") {
          e.preventDefault();
          e.stopPropagation();
          this.speakCurrentVocab(state.currentCard);
          return;
  ...
  private renderReviewSession(parent: HTMLElement) {
    ...
    if (!state.isFlipped) {
      ...
      this.renderRelatedCards(parent, card);
    }
  ...
  private renderRatingArea(parent: HTMLElement, card: RecallCard) {
    ...
    area.style.borderTop = "1px solid var(--background-modifier-border)";
  ...
  private async renderRelatedCards(parent: HTMLElement, card: RecallCard) {
    const relatedContainer = parent.createDiv({ cls: "mindos-recall-related-container" });
    ...
    const relationView = new CardRelationView(
      ...
      this.plugin.cardRelationService,
      {
        onOpenCard: (relatedCard) => {
          this.showRelatedCardDetail(relatedCard);
        },
        onQuickReview: (relatedCard) => {
          this.quickReviewCard(relatedCard);
        },
        onRefresh: () => {
          this.plugin.recallStore.refresh();
        },
      }
    );

    await relationView.render(card, 3);
  }

  /**
   * ✅ 显示相关卡片详情（美观弹窗）
   */
  private showRelatedCardDetail(relatedCard: RecallCard) {
    ...
    class CardDetailModal extends Modal {
      constructor(private plugin: MindOSPlugin, private card: RecallCard) {
        super(plugin.app);
        this.modalEl.addClass("mindos-card-detail-modal");
      }

      onOpen() {
        const { contentEl } = this;
        contentEl.empty();
        contentEl.addClass("mindos-card-detail-content");

        const card = this.card as RecallCard;
        const scenario = (card as any).scenario || "unknown";

        // vocab 场景走单词学习专属布局
        if (scenario === "vocab") {
          this.renderVocabLayout(contentEl, card);
          return;
        }
        ...
        const header = contentEl.createDiv({ cls: "mindos-card-detail-header" });
        const headerLeft = header.createDiv({ cls: "mindos-card-detail-header-left" });
        const iconWrap = headerLeft.createDiv({ cls: "mindos-card-detail-scenario-icon" });
        setIcon(iconWrap, scenarioMeta.icon);
        const tagEl = headerLeft.createSpan({ cls: "mindos-card-detail-scenario-tag" });
        tagEl.setText(scenarioMeta.label);
        const closeBtn = header.createEl("button", { cls: "mindos-card-detail-close" });
        setIcon(closeBtn, "x");
        closeBtn.onclick = () => this.close();

        const frontWrap = contentEl.createDiv({ cls: "mindos-card-detail-front-wrap" });
        const frontEl = frontWrap.createDiv({ cls: "mindos-card-detail-front" });
        try {
          const mdComp = new Component();
          MarkdownRenderer.render(this.plugin.app, card.front, frontEl, "", mdComp);
        } catch {
          frontEl.setText(card.front);
        }

        contentEl.createDiv({ cls: "mindos-card-detail-divider" });

        const body = contentEl.createDiv({ cls: "mindos-card-detail-body" });
        const backLabel = body.createDiv({ cls: "mindos-card-detail-section-label" });
        setIcon(backLabel.createSpan(), "check-circle");
        backLabel.createSpan({ text: " 标准答案" });
        const backEl = body.createDiv({ cls: "mindos-card-detail-back" });
        try {
          const mdComp = new Component();
          MarkdownRenderer.render(this.plugin.app, card.back, backEl, "", mdComp);
        } catch {
          backEl.setText(card.back);
        }

        this.renderExtras(body, card);

        const metaWrap = contentEl.createDiv({ cls: "mindos-card-detail-meta" });
        this.renderMeta(metaWrap, card);

        const footer = contentEl.createDiv({ cls: "mindos-card-detail-footer" });
        const okBtn = footer.createEl("button", { cls: "mindos-card-detail-btn" });
        okBtn.setText("知道了");
        okBtn.onclick = () => this.close();
      }

      /* ────── 单词学习专属布局 ────── */
      private renderVocabLayout(container: HTMLElement, card: RecallCard) {
        const entry = this.parseVocabEntry(card);

        // ═══ 顶部导航条 ───
        const topBar = container.createDiv({ cls: "mindos-vocab-topbar" });
        const topLeft = topBar.createDiv({ cls: "mindos-vocab-topbar-left" });
        const iconWrap = topLeft.createDiv({ cls: "mindos-vocab-scenario-icon" });
        setIcon(iconWrap, "languages");
        topLeft.createSpan({ cls: "mindos-vocab-scenario-label", text: "单词学习" });

        const closeBtn = topBar.createEl("button", { cls: "mindos-vocab-close" });
        setIcon(closeBtn, "x");
        closeBtn.onclick = () => this.close();

        // ═══ 单词主体卡 ═══
        const wordCard = container.createDiv({ cls: "mindos-vocab-word-card" });

        // 单词 + 发音图标
        const wordRow = wordCard.createDiv({ cls: "mindos-vocab-word-row" });
        const wordEl = wordRow.createSpan({ cls: "mindos-vocab-word" });
        wordEl.setText(entry.word || card.front || "");

        // 小喇叭发音按钮
        const speakerBtn = wordRow.createEl("button", { cls: "mindos-vocab-speaker" });
        setIcon(speakerBtn, "volume-2");
        speakerBtn.onclick = (e: MouseEvent) => {
          e.stopPropagation();
          this.speakWord(entry.word);
        };

        // 音标（斜体）
        if (entry.phonetic) {
          const phoneticEl = wordCard.createDiv({ cls: "mindos-vocab-phonetic" });
          phoneticEl.setText(entry.phonetic);
        }

        // 词性
        if (entry.partOfSpeech) {
          const posEl = wordCard.createDiv({ cls: "mindos-vocab-pos" });
          posEl.setText(entry.partOfSpeech);
        }

        // 分隔
        wordCard.createDiv({ cls: "mindos-vocab-divider" });

        // 释义
        if (entry.definitions && entry.definitions.length > 0) {
          const defsSection = wordCard.createDiv({ cls: "mindos-vocab-defs" });
          const defsLabel = defsSection.createDiv({ cls: "mindos-vocab-defs-label" });
          defsLabel.setText("释义");

          const defsList = defsSection.createDiv({ cls: "mindos-vocab-defs-list" });
          for (let i = 0; i < entry.definitions.length; i++) {
            const defItem = defsList.createDiv({ cls: "mindos-vocab-def-item" });
            const idxSpan = defItem.createSpan({ cls: "mindos-vocab-def-idx" });
            idxSpan.setText(String(i + 1));
            defItem.createSpan({ cls: "mindos-vocab-def-text", text: entry.definitions[i] });
          }
        }

        // 英文释义
        if (entry.englishDef) {
          const engDef = wordCard.createDiv({ cls: "mindos-vocab-english-def" });
          engDef.setText(entry.englishDef);
        }

        // ═══ 例句区 ═══
        if (entry.examples && entry.examples.length > 0) {
          const exSection = container.createDiv({ cls: "mindos-vocab-example-section" });
          const exLabel = exSection.createDiv({ cls: "mindos-vocab-section-title" });
          setIcon(exLabel.createSpan(), "message-square");
          exLabel.createSpan({ text: " 例句" });

          for (const ex of entry.examples) {
            const exItem = exSection.createDiv({ cls: "mindos-vocab-example-item" });
            exItem.createDiv({ cls: "mindos-vocab-example-en", text: ex.en });
            if (ex.cn) {
              exItem.createDiv({ cls: "mindos-vocab-example-cn", text: ex.cn });
            }
          }
        }
        ...
        if (entry.synonyms && entry.synonyms.length > 0) {
          const synSection = container.createDiv({ cls: "mindos-vocab-syn-section" });
          const synLabel = synSection.createDiv({ cls: "mindos-vocab-section-title" });
          setIcon(synLabel.createSpan(), "link");
          synLabel.createSpan({ text: " 近义词" });
          const tagsWrap = synSection.createDiv({ cls: "mindos-vocab-tags-row" });
          for (const syn of entry.synonyms) {
            tagsWrap.createSpan({ cls: "mindos-vocab-chip", text: syn });
          }
        }

        // ═══ 答案区（可折叠） ═══
        if (card.back) {
          const answerSection = container.createDiv({ cls: "mindos-vocab-answer-section" });
          const answerTitle = answerSection.createDiv({ cls: "mindos-vocab-section-title is-answer" });
          setIcon(answerTitle.createSpan(), "check-circle");
          answerTitle.createSpan({ text: " 标准答案" });
          const answerBody = answerSection.createDiv({ cls: "mindos-vocab-answer-body" });
          try {
            const mdComp = new Component();
            MarkdownRenderer.render(this.plugin.app, card.back, answerBody, "", mdComp);
          } catch {
            answerBody.setText(card.back);
          }
        }

        // ═══ 提示 ═══
        if (card.hints && card.hints.length > 0) {
          const hintSection = container.createDiv({ cls: "mindos-vocab-hint-section" });
          const hintTitle = hintSection.createDiv({ cls: "mindos-vocab-section-title" });
          setIcon(hintTitle.createSpan(), "lightbulb");
          hintTitle.createSpan({ text: " 提示" });
          for (const h of card.hints) {
            hintSection.createDiv({ cls: "mindos-vocab-hint-item", text: h });
          }
        }

        // ═══ 底部元信息 ═══
        const metaWrap = container.createDiv({ cls: "mindos-vocab-meta" });

        if (card.tags && card.tags.length > 0) {
          const tagsRow = metaWrap.createDiv({ cls: "mindos-vocab-tags-row" });
          for (const tag of card.tags) {
            tagsRow.createSpan({ cls: "mindos-vocab-chip is-tag", text: tag });
          }
        }
        ...
        // ═══ 底部按钮 ═══
        const footer = container.createDiv({ cls: "mindos-vocab-footer" });
        const okBtn = footer.createEl("button", { cls: "mindos-vocab-ok-btn" });
        okBtn.setText("知道了");
        okBtn.onclick = () => this.close();
      }

      // 解析 vocab 卡片的词条数据
      private parseVocabEntry(card: RecallCard) {
        const meta = (card as any).metadata || {};
        const entry = meta.entry || {};
        return {
          word: meta.word || entry.word || card.front || "",
          phonetic: meta.phonetic || entry.phonetic || "",
          partOfSpeech: meta.partOfSpeech || entry.partOfSpeech || "",
          definitions: (entry.definitions as string[]) || [],
          englishDef: (entry.englishDef as string) || "",
          examples: (entry.examples as Array<{ en: string; cn?: string }>) || [],
      ...
      /* ────── 通用：附加内容 ────── */
      private renderExtras(body: HTMLElement, card: RecallCard) {
        if (card.hints && card.hints.length > 0) {
          const hintsWrap = body.createDiv({ cls: "mindos-card-detail-hints" });
          const hintsLabel = hintsWrap.createDiv({ cls: "mindos-card-detail-section-label" });
          setIcon(hintsLabel.createSpan(), "lightbulb");
          hintsLabel.createSpan({ text: " 提示" });
          const hintsList = hintsWrap.createDiv({ cls: "mindos-card-detail-hints-list" });
          for (const hint of card.hints) {
            hintsList.createDiv({ cls: "mindos-card-detail-hint-item", text: hint });
          }
        }
        if (card.examples && card.examples.length > 0) {
          const exWrap = body.createDiv({ cls: "mindos-card-detail-examples" });
          const exLabel = exWrap.createDiv({ cls: "mindos-card-detail-section-label" });
          setIcon(exLabel.createSpan(), "file-text");
          exLabel.createSpan({ text: " 示例" });
          const exList = exWrap.createDiv({ cls: "mindos-card-detail-examples-list" });
          for (const ex of card.examples) {
            const exItem = exList.createDiv({ cls: "mindos-card-detail-example-item" });
            try {
              const mdComp = new Component();
              MarkdownRenderer.render(this.plugin.app, ex, exItem, "", mdComp);
            } catch {
              exItem.setText(ex);
            }
          }
      ...
      private renderMeta(metaWrap: HTMLElement, card: RecallCard) {
        if (card.tags && card.tags.length > 0) {
          const tagsWrap = metaWrap.createDiv({ cls: "mindos-card-detail-tags" });
          for (const tag of card.tags) {
            tagsWrap.createSpan({ cls: "mindos-card-detail-tag", text: tag });
          }
        }
        if (card.sourcePath) {
          const sourceWrap = metaWrap.createDiv({ cls: "mindos-card-detail-source" });
          setIcon(sourceWrap.createSpan(), "file-text");
          const sp = sourceWrap.createSpan({ cls: "mindos-card-detail-source-path", text: this.shortenPath(card.sourcePath) });
          sp.onclick = () => this.plugin.openFile(card.sourcePath!);
        }
        const srs = card.srs;
        if (srs) {
      ...
      private getScenarioMeta(scenario: string): { label: string; icon: string } {
        const meta: Record<string, { label: string; icon: string }> = {
          vocab: { label: "单词", icon: "languages" },
          qa: { label: "问答", icon: "help-circle" },
          code: { label: "代码", icon: "code-2" },
          concept: { label: "概念", icon: "lightbulb" },
          interview: { label: "面试", icon: "briefcase" },
          command: { label: "命令", icon: "terminal" },
          phrase: { label: "短语", icon: "message-circle" },
          wiki: { label: "Wiki", icon: "book-open" },
          custom: { label: "自定义", icon: "settings" },
          unknown: { label: "卡片", icon: "file-text" },
        };
        return meta[scenario] || meta.unknown;
      }
    }

    new CardDetailModal(this.plugin, relatedCard).open();
  }

  private quickReviewCard(card: RecallCard) {
    const state = this.plugin.recallStore.getState();
    const hasActiveSession = state.currentSession && state.currentCard;

    if (hasActiveSession) {
      const confirmed = confirm(`当前正在复习会话中，是否要中断并快速复习这张卡片？`);
      if (!confirmed) return;
      this.plugin.recallStore.finishSession();
      this.plugin.currentRecallCards = [];
    }

    this.plugin.currentRecallCards = [card];
    const session = {
      id: `quick_${card.id}`,
    ...
    this.plugin.recallStore.startSession(session);
    this.plugin.recallStore.setCurrentCard(card, 0);
    this.cardStartTime = Date.now();
    this.answerInputCache = "";
  }
  ...
  private openWordListManager() {
    // 动态 import 避免循环依赖
    import("../vocab/word-list-manager-modal").then(({ WordListManagerModal }) => {
  ...
  private openConceptGenerator() {
    import("../concept-phrase/concept-phrase-modal").then(({ ConceptGenerateModal }) => {
  ...
  private openPhraseGenerator() {
    import("../concept-phrase/concept-phrase-modal").then(({ PhraseGenerateModal }) => {
  ...
  private openCustomScenarioManager() {
    import("../custom-scenario/custom-scenario-modals").then(({ CustomScenarioListModal }) => {
  ...
  private openDashboard() {
    import("../dashboard/dashboard-view").then(({ DashboardView }) => {
