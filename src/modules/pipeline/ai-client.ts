...
export class AIClient {
  ...
  async chat(
    ...
    return await this.callApiWithRetry(systemPrompt, userMessage);
  }
...
【Obsidian Markdown 格式规范 — 必须严格遵守】
- 标题：用 ## 和 ### ，不要用 # 或 #### 以上
- 强调：用 **粗体** 和 *斜体*，不要用 __underline__ 或 ==highlight==
- Callout（摘要/提示框）：严格用 > [!type] 标题 格式
  ✅ 正确：> [!abstract] 简介\\n> 内容...
  ❌ 错误：| ![[abstract]] 简介（这不是合法的 Obsidian 语法）
  ❌ 错误：> ![[abstract]]（双括号用于文件引用，不用于 callout）
- 列表：用 - 或 1. 有序列表，保持缩进一致
- 代码块：用三个反引号(语言名)包裹
- 不要在内容中输出 frontmatter（--- 包裹的 YAML），插件会自动生成
- 不要在内容中使用 ![[图片]] 引用语法（插件会单独处理图片追加）`;