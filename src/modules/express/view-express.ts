...
import { EXPRESS_LENGTH_MAP, PLUGIN_NAME } from '../../core/constants';
...
export class ExpressView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf, plugin: MindOSPlugin) {
    ...
    this.store     = new ArticleStore(plugin.app, () => (plugin as any).settings?.baseFolder || '');
  ...
  async onOpen(): Promise<void> {
    ...
    this.rootEl.empty();
    this.rootEl.addClass('mindos-root', 'mindos-express');
  ...
  private renderHeader(parent: HTMLElement): void {
    ...
    // 左：页面标识（仅文字，品牌已在 topbar）
    const left = header.createDiv({ cls: 'express-header-left' });
    left.createEl('span', { cls: 'express-header-title', text: '写作工作台' });
...
private async handleGenerateArticle(): Promise<void> {
  ...
  private async handleExportToWiki(): Promise<void> {
    ...
    try {
      ...
      const exportRel  = (settings?.expressExportFolder as string) || '知识库/文章';
