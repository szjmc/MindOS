...
export class MindOSRetrieveView extends ItemView {
  ...
  render() {
    ...
    let tab = this.plugin.retrieveStore.getState().currentTab;
    // 兼容已移除的 express / 未知 tab → 回退到 capture
    if (!["capture", "search", "chat", "recall"].includes(tab)) {
      tab = "capture";
      this.plugin.retrieveStore.setTab(tab);
    }
    if (tab === "capture") {
  ...
  private renderTopBar(parent: HTMLElement) {
    ...
    const center = bar.createDiv({ cls: "mindos-topbar-center" });
    // 智能分析按钮已移到左侧 ribbon
  ...
  private closeAllDropdowns() {
    document.querySelectorAll(".mindos-analysis-dropdown.is-open").forEach(el => {
