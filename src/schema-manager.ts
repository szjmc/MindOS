...
const CLAUDE_MD_TEMPLATE = `# CLAUDE.md - LLM Wiki 工作说明书
...
你必须输出 JSON 格式的 actions 数组，每个 action 形如：

\`\`\`json
{
  "op": "create" | "update" | "append_section" | "link",
  "pageType": "entity" | "concept" | "topic" | "comparison" | "overview",
  "title": "页面标题（用作文件名）",
  "brief": "一句话简介（≤30字，必填）",
  "tags": ["tag1", "tag2"],
  "content": "Markdown 正文（不含 frontmatter，不含 # 标题行）",
  "section": "如果是 append_section，指定要追加到的小节名",
  "links": ["[[已有页面1]]", "[[已有页面2]]"],
  "reason": "你为什么要执行这个 action（1-2 句）"
}
\`\`\`
